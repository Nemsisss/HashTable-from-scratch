...
bool HashTable<K,V,Prober,Hash,KEqual>::empty() const
{
    for(long unsigned int i=0; i<table_.size();i++)
    {
        if(this->table_[i]!=nullptr)
        {
            if(this->table_[i]->deleted==0) //if an item exists that is not deleted then the table_ is not empty
            {
                return false;
            }
        }
...
{
    currAlpha= (double)occupied/table_.size();
    if(this->currAlpha >= this->resize_alpha)
    {
        resize();
    }
    if( probe(p.first)==npos) 
    {
        throw std::logic_error("Item cannot be inserted");
    }
    int index= probe(p.first);
    if(table_[index]==nullptr)
    {
        HashItem* newItem= new HashItem(p);
        table_[index]=newItem;
        occupied++;
    }
    else
    {
        table_[index]->item.second= p.second; //if the key already exists overwrite its value
    }
}
...
void HashTable<K,V,Prober,Hash,KEqual>::remove(const KeyType& key)
{
    if(!this->empty())
    {
         HashItem* toBeRemoved= this->internalFind(key);
        if(toBeRemoved!=nullptr)
        {
            toBeRemoved->deleted=1;
        }
    }
}
...
{
     vector<HashItem*> tempVect=table_ ;
     mIndex_++;
     table_.assign(CAPACITIES[mIndex_], nullptr);
     for(long unsigned int i=0; i<tempVect.size();i++)
     {
         if(tempVect[i]!=nullptr)
         {
             if(tempVect[i]->deleted==0)
             {
                 int index=probe(tempVect[i]->item.first);
                 table_[index]=tempVect[i];
             }
             else
             {
                 occupied--;
...
HASH_INDEX_T HashTable<K,V,Prober,Hash,KEqual>::probe(const KeyType& key) const
{
    ...
    while(Prober::npos != loc)
    {
        ...
        else if( table_[loc]->item.first ==key &&  table_[loc]->deleted!=1) 
        {
            return loc;
        }
        loc = prober_.next();
